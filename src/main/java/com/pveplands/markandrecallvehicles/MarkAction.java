...
public class MarkAction implements ActionPerformer, BehaviourProvider, ModAction {
    private static Logger logger = Logger.getLogger(MarkAndRecallVehicles.getLoggerName(MarkAction.class));
    ...
    public boolean action(@Nonnull Action action, @Nonnull Creature performer, @Nonnull Item target, short num, float counter) {
        ...
        if (counter == 1f) {
            ...
            if (counter * 10f > action.getTimeLeft()) {
                // logger.info(String.format("%s marked a rune (%d) at x: %d y: %d layer: %d floorLevel: %d",
                //         performer.getName(), target.getWurmId(), performer.getTileX(), performer.getTileY(), performer.getLayer(), performer.getFloorLevel()));
