...
import com.pveplands.markandrecallvehicles.TeleportHelper;

public class RecallAction implements ActionPerformer, ModAction {
    private static final Random random = new Random();
    private static final Logger logger = Logger.getLogger(MarkAndRecallVehicles.getLoggerName(RecallAction.class));
    ...
    public boolean action(@Nonnull Action action, @Nonnull Creature performer, @Nonnull Item target, short num, float counter) {
        ...
        if (counter == 1f) {
            if (performer.getVehicle() != -10 && target.getCurrentQualityLevel() < options.getMinUsableQualityForVehicle()) {
                performer.getCommunicator().sendNormalServerMessage("The rune is in too poor shape to be used while embarked.");
                return true;
            }
            ...
            if (counter * 10f > action.getTimeLeft() + 10) {
                ...
                TeleportHelper.doTeleport(performer, x, y, layer, floorLevel);
